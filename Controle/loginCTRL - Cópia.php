...
switch ($evento) {
    case 'autenticar': {
            ...
            $senhaRede = Funcoes::noInjection($_POST['senhaRede']);
            ...
            //conexão AD
            if (!$lnkConexao) {
                echo("Não há conexão com o AD Odebrecht.net");
                return;
            }
            
            //autenticação no AD
            $ldapBind = @ldap_bind($lnkConexao, $loginRede . '@odebrecht.net', $senhaRede);
			
			if ($ldapBind == false) {
				$lnkConexao = @ldap_connect('10.156.100.83');
				$ldapBind = @ldap_bind($lnkConexao, $loginRede . '@odebrecht.net', $senhaRede) or die( ldap_error($lnkConexao) );
			}
			...
			if ( $atributos['count'] == 0 ) {
				$resultado = @ldap_search($lnkConexao, 'OU=Ambiental,DC=odebrecht,DC=net', "(userprincipalname=$loginRede*)") ;
				$atributos = ldap_get_entries($lnkConexao, $resultado);
			}		
            ...
            if ($integrante == null) {
                echo("Verifique o nome e o cpf cadastrado no Active Directory e no Sistema Vênus" );
                return;
            }
